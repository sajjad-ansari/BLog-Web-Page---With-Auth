...
export const createUser = (req, res) => {
  ...
  db.query(sql, [email], (err, result) => {
    // Correctly passing email as an array
    if (err) {
      return res.send(err);
    }
    ...
    if (result.length > 0) {
      ...
      db.query(sql, [phone], (err, data) => {
        // Correctly passing phone as an array
        if (err) {
          return res.send(err);
        }
        ...
        if (data.length > 0) {
          ...
          bcrypt.hash(password, 10, (err, hash) => {
            const sql =
              "INSERT INTO user(f_name, l_name, address, email, password, phone, gender, dob, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//       return res
//         .status(200)
//         .send("Profile picture uploaded to database Successfully!!");
//     }
//   });
// };
